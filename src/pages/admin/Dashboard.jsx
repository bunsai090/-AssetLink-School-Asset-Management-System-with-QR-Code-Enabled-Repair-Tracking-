...
import {
    ...
    ArrowRight, CalendarDays, TrendingUp, Activity,
...
const PRIORITY_CONFIG = {
    Critical: { color: 'bg-red-500', dot: 'text-red-500', badge: 'destructive' },
    High:     { color: 'bg-orange-400', dot: 'text-orange-400', badge: 'warning' },
    Medium:   { color: 'bg-amber-400', dot: 'text-amber-400', badge: 'secondary' },
    Low:      { color: 'bg-emerald-400', dot: 'text-emerald-400', badge: 'outline' },
};
...
function StatCard({ title, value, subtitle, icon: Icon, colorClass, loading }) {
    return (
        <Card className="relative overflow-hidden border-border/60 shadow-sm hover:shadow-md transition-shadow duration-300">
            ...
            <div className={`absolute bottom-0 left-0 right-0 h-0.5 ${colorClass.replace('bg-', 'bg-').replace('/10', '')}`} />
        </Card>
    );
}
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2 border-border/60 shadow-sm">
                    <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Link to="/repair-requests">
                                <Button variant="ghost" size="sm" className="text-[#028a0f] hover:text-[#016d0c] hover:bg-emerald-50 gap-1 text-xs font-bold">
                            ...
                            pendingApproval.slice(0, 6).map((req, idx) => {
                                ...
                                return (
                                    <Link key={req.id} to="/repair-requests">
                                        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-accent/60 transition-colors duration-150 cursor-pointer group">
                                            {/* Priority stripe */}
                                            <div className={`w-1 h-10 rounded-full flex-shrink-0 ${priority.color}`} />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-semibold text-foreground truncate group-hover:text-[#028a0f] transition-colors">
                                                    {req.asset_name}
                                                </p>
                                                <p className="text-xs text-muted-foreground truncate">{req.description}</p>
                                                <p className="text-[10px] text-muted-foreground/70 mt-0.5 font-medium uppercase tracking-wide">
                                            ...
                                            <ChevronRight className="w-4 h-4 text-muted-foreground/30 group-hover:text-[#028a0f] transition-colors flex-shrink-0" />
                                        </div>
                                    </Link>
                                );
                            })
                ...
                {/* Right column — Progress + Quick Stats */}
                                    ...
                                    {[
                                        {
                                            ...
                                            value: requests.length > 0 ? `${Math.round((completed.length / requests.length) * 100)}%` : '0%',
                                            color: 'text-emerald-600'
                                        },
                                        {
                                            label: 'Pending Actions',
                                            value: pendingApproval.length,
                                            color: pendingApproval.length > 0 ? 'text-amber-600' : 'text-emerald-600'
                                        },
                                        {
                                            label: 'Scheduled Today',
                                            value: todayScheduled.length,
                                            color: todayScheduled.length > 0 ? 'text-blue-600' : 'text-muted-foreground'
                                        },
